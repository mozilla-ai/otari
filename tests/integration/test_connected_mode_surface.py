...
def test_connected_mode_starts_without_database(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_connected_mode_disables_local_management_endpoints(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OTARI_AI_TOKEN", "gw_test_token")
    ...
    expected = {"detail": "This endpoint is not available in connected mode. Manage this resource via the platform UI."}
...
def test_connected_mode_health_reports_reachability(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OTARI_AI_TOKEN", "gw_test_token")
    ...
    assert response.status_code == 200
    assert response.json() == {"status": "healthy", "mode": "connected", "platform_reachable": "yes"}
...
def test_connected_mode_readiness_fails_when_platform_unreachable(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OTARI_AI_TOKEN", "gw_test_token")
