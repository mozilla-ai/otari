...
async def test_authenticated_protocol_prepares_and_finalizes(
    ...
    async def authenticate(request: Request, db: AsyncSession) -> FileScope:
    ...
    async def authorize(scope: FileScope, body: OutputPrepare, db: AsyncSession) -> FileAccount:
        return account
    ...
    headers = {"X-Gateway-Token": "gateway", "X-User-Token": "user"}
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://control") as client:
        ...
        assert denied.status_code == 401 and denied.headers["X-Otari-Files-Protocol"] == "1"
        ...
        listing = await client.post(f"{API_ROOT}/gateway/files/list", json={}, headers=headers)
