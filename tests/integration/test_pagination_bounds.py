...
def test_users_list_accepts_valid_bounds(client: TestClient, master_key_header: dict[str, str]) -> None:
    ...
    response = client.get(f"{API_ROOT}/users?skip=0&limit=100", headers=master_key_header)
    assert response.status_code == 200

    response = client.get(f"{API_ROOT}/users?skip=0&limit=1000", headers=master_key_header)
    assert response.status_code == 200
