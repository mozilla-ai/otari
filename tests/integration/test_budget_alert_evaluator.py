...
of the budget period and a contributed background task runs once per
...
from otari_alerts.destinations import redact_alert_destination
from otari_alerts.dispatcher import AlertDispatchResult
from otari_alerts.evaluator import (
    KIND_EXCEEDED,
    KIND_WARNING,
...
def sent(monkeypatch: pytest.MonkeyPatch) -> list[tuple[str, str]]:
    ...
    monkeypatch.setattr("otari_alerts.evaluator.send_alert", _capture)
    return captured
...
def failing_send(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("otari_alerts.evaluator.send_alert", _fail)
...
async def test_two_concurrent_passes_send_exactly_one(
    ...
    A contributed background task runs once per worker, so this is the
