...
Every error raised here is a subclass of Otari's ``TenancyError``, which is
the whole of how the plugin's statuses are registered: one handler in
``gateway.main`` renders the family, and each class carries its own
``status_code``, so a plugin declaring a subclass out of tree needs no
