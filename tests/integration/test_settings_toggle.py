...
def test_get_settings_reports_toggle_flags(postgres_url: str, master_header: dict[str, str]) -> None:
    ...
    try:
        body = client.get(f"{API_ROOT}/settings", headers=master_header).json()
...
def test_patch_toggles_and_applies_settings(postgres_url: str, master_header: dict[str, str]) -> None:
    ...
    try:
        ...
        assert client.get(f"{API_ROOT}/settings", headers=master_header).json()["model_discovery"] is False
    finally:
        client_gen.close()
...
def test_patch_leaves_omitted_fields_unchanged(postgres_url: str, master_header: dict[str, str]) -> None:
    ...
    client = next(client_gen)
    try:
        body = client.patch(f"{API_ROOT}/settings", json={"default_pricing": True}, headers=master_header).json()
        assert body["default_pricing"] is True
...
def test_override_survives_a_restart(postgres_url: str, master_header: dict[str, str]) -> None:
    ...
    try:
        first.patch(f"{API_ROOT}/settings", json={"model_discovery": False}, headers=master_header)
    finally:
        first_gen.close()
    ...
    try:
        assert second.get(f"{API_ROOT}/settings", headers=master_header).json()["model_discovery"] is False
    finally:
        second_gen.close()
...
def test_patch_requires_master_key(postgres_url: str) -> None:
    ...
    client = next(client_gen)
    try:
        resp = client.patch(f"{API_ROOT}/settings", json={"model_discovery": False})
