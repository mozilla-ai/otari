...
provider's ID, with Anthropic's Files API stubbed here at the HTTP level. A row
recorded without a copy has no ``storage_ref`` and is streamed from the
provider on demand, with that provider call faked.
"""
