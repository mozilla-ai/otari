...
These go through all three completion endpoints with the provider call patched
out and the vendor client stubbed, so what is asserted is admission: whether a
check ran at all, and what its verdict did to the request.
...
def test_an_enabled_definition_blocks_a_request_that_asked_for_nothing(
    ...
    assert built(lambda: get_guardrail_runner().knows(_PROFILE))
