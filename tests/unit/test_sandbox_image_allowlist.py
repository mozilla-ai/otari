...
def test_the_deployment_image_is_pinnable_without_being_listed() -> None:
    ...
    config = GatewayConfig(sandbox_image="mzdotai/otari-sandbox-container:latest")
...
def test_the_curated_list_is_split_trimmed_and_deduped() -> None:
    config = GatewayConfig(
        sandbox_image="mzdotai/otari-sandbox-container:latest",
        sandbox_allowed_images=" ghcr.io/acme/sandbox:2 ,mzdotai/otari-sandbox-container:latest,, ghcr.io/acme/gpu:1",
    )
...
def test_a_curated_list_without_a_deployment_image_stands_on_its_own() -> None:
    config = GatewayConfig(sandbox_allowed_images="ghcr.io/acme/sandbox:2")
    assert config.pinnable_sandbox_images() == ("ghcr.io/acme/sandbox:2",)


def test_a_blank_list_is_the_same_as_none() -> None:
    assert GatewayConfig(sandbox_allowed_images="  ,  ").pinnable_sandbox_images() == ()
