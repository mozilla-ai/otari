...
same profile is checked once and cannot weaken it, and that it is the outermost
layer, so it owns the endpoint where a policy or an organization names the same
profile.
...
def test_a_stored_definition_beats_a_routing_policy_entry_of_the_same_name() -> None:
    """The operator wrote both, and the definition is the more explicit one: built here, switched on here."""
