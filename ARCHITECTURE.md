...
A **core feature package** is the unit a feature of the base ships as: one package under `src/gateway/` owning its routes, service, models, errors, settings, metrics, worker and tests, listed in `src/gateway/packages.py`. That list is a literal tuple edited by hand; core's router registration, the lifespan and the deployment bootstrap read it. A listed package mounts as core routes, ungated, and hosts its surface when its own setting enables it. The bootstrap hook is not involved: `OTARI_BOOTSTRAP` selects an edition, and never switches a core feature. Nothing is discovered from installed packages, and a package never registers itself on import.
