...
- **Entitlement** is the licensing axis: *is this capability enabled for this deployment at all?* It is scoped per deployment, never per user. It is resolved by `EntitlementPort`, whose core adapter grants every base capability plus every capability a contributed router installs, and reports everything else as absent; a real resolver is an overlay adapter.
...
Be clear about how much of that is built. Both axes are served by the same payload: `GET /api/v1/bootstrap` answers `surfaces` (topology) and `capabilities` (entitlement, sorted, whatever `EntitlementPort` resolves for this deployment). The browser does not read the second one yet, so the two halves still answer independently:

- **In the browser**, the axis resolves from the constant that is the default value of the context in `web/src/shared/hooks/useEntitlements.tsx`. It grants `BASE_CAPABILITIES` and reports everything else absent. An overlay answers it for real by rendering `EntitlementProvider`, through `web/src/app/overlayEntitlementResolver.tsx`: the seam the shell mounts above the navigation and the routes, whose base default renders its children unchanged so this build falls through to the constant. A resolver that answers asynchronously reports `isLoading` and the shell waits on it, rather than telling a visitor an entitled page is not served here while the answer is still in flight. Wiring this hook to the bootstrap's `capabilities` is the follow-up that lets a plugin's nav item appear without a browser-side resolver.
- **On the server**, `EntitlementPort` resolves through the container. Its core adapter (`src/gateway/adapters/entitlement_adapter.py`) answers with its own `BASE_CAPABILITIES` plus every capability the container's router contributions name: a bootstrap that only adds a surface has installed that capability, and with no real resolver bound, installed is the only evidence the base has, so it grants what it mounts. An overlay that binds a real resolver replaces the adapter and decides for itself. That answer is what `require_capability` gates a contributed router on, so a route an overlay mounts into this process refuses for itself rather than trusting a hidden link. Hiding a link is not authorization; this is the half that is.

Both `BASE_CAPABILITIES` constants are empty, and deliberately so: no base nav entry and no base route is gated on a capability, because the one candidate is routing, whose split this document still marks provisional. They are meant to agree, so a capability the base grows is added to both at once.
