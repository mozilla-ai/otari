...
A **core feature** is one domain-named module per layer (route, schema, service, repository, exceptions, model) plus one registry entry in `src/gateway/features.py`. That list is a literal tuple edited by hand; core's router registration, the lifespan and the deployment bootstrap read it. A listed feature mounts as core routes, ungated, and hosts its surface when its own setting enables it. The bootstrap hook is not involved: `OTARI_BOOTSTRAP` selects an edition, and never switches a core feature. Nothing is discovered from installed packages, and a feature never registers itself on import.
