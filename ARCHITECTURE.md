...
**What the boundary check enforces.** `scripts/check_architecture.py` enforces part of the rules above today. It refuses a service that imports the API layer or an adapter, a repository that imports a service or the API layer, and a route that imports `sqlalchemy.orm`. Of rules 1 and 2 it enforces two parts: a route or a service builds no query, and a module-level function under `services/` takes no `AsyncSession`. Those two start from baselines, `QUERY_BASELINE` and `SESSION_PARAMETER_BASELINE`, which name the code still in the old shape, and a baseline only shrinks. It also refuses a new top-level module in `services/` or `repositories/`, so new code goes in its domain's package, starting from `FLAT_MODULE_BASELINE`, the flat modules still in those layers. The rest of rules 1 and 2, and rules 3 to 6, are review rules until the check covers them. No check can tell whether a service's API is small, so review carries that rule too. New and moved code follows the target shape, and never copies code named on a baseline.
...
1. **Write the feature's modules** in its domain's shape, as [The modular monolith](#the-modular-monolith) lays it out. One of them declares the feature's `CoreFeature` (`src/gateway/core/feature.py`). None of them imports `src/gateway/features.py`: the boundary check refuses a service or a route that imports the registry. Its queries go in its repository package, because the boundary check also refuses a route or a service that builds one. Its service is built with its own repositories and a Unit of Work, and never receives the database session. The boundary check refuses a module-level service function that takes one.
