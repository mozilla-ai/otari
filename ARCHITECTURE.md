...
- **Surfaces exposed to callers: change it by adding a surface.** The API routes and frontend pages that callers reach. Each is registered into a central list the core owns (a router list on the backend, a nav registry on the frontend). A frontend page is switched on by an entitlement. A backend router is switched on by one only when it declares a capability: a contributed router whose `capability` is `None` mounts unconditionally, which is what a plugin that is either installed or not wants, since it has nothing to license. Extending here means registering a new route or page, whether that surface ships in Otari's core or comes from an overlay. Nothing is swapped; a surface is added.
