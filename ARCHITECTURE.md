...
| `HostedGuardrailPort` | Offering deployment-owned guardrails an organization may pick, and running and metering their checks. |
...
| Hosted guardrails | **Core port + hosted adapter** | An organization's own guardrail definitions and mandates are core; deployment-owned guardrails every organization may pick, with their vendor secrets and per-check metering, come from an overlay. |
