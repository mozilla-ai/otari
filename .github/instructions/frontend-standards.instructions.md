...
8. **Navigation is data, and its three gates stay three.** A *rail* destination is declared in
   ...
   component. Playground, documentation, and account routes are chrome rather than navigation and are hand-written `Link`s on
   purpose, so do not flag them or ask for them to be registered: `/playground` and `/docs` (top bar, and the
   account menu below `md`) and `/account` (that menu's first row). The registry is what the
   rails render, so putting them in a sidebar section would duplicate into the sidebar a row the design draws
   once. Playground remains in route lookup to preserve its surface gate. An entry gates on `surface` (the deployment axis, from `GET /v1/bootstrap`
