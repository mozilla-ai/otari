...
- **The client `user` field is not trusted for non-master keys.** Spend is bound
  ...
  (403); set `ignore_user_mismatch` on a single key, or
  `reject_user_mismatch=false` deployment-wide, to instead bind to the key's own
  user while still forwarding `user` to the provider as an end-user tag.
