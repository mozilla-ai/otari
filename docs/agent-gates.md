...
under check, deliberately.** A repo-local script named by a repo-controlled
policy file is the same trust level as a Makefile target or a pre-commit hook
already checked into the repo: nothing new. `otari hook` already runs
whatever the checked-out `.otari-gates.yml` names, on the machine of whoever
has the hook installed, the moment that policy is evaluated; running a
repo-local *script* it names is not a materially different capability. A
"verifier must predate this diff" rule was considered and rejected
...
introduces it. Running `otari hook` against a branch you do not already
trust carries the same risk running any of its other scripts on that branch
would; this gate type does not introduce a new one.
