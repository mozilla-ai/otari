...
The last three say what the definition does once it is switched on, and every
one of them defaults to the value above. They are described under
[what an enabled definition does to a request](#what-an-enabled-definition-does-to-a-request);
`applies_to_all_workspaces` is the exception that defaults to `false`, so a
definition reaches no workspace until one is named or that flag is set.
...
stopping every request the definition covers.
...
organization, the deployment's own stored definitions, and a
[routing policy](routing.md) the operator wrote. They are merged by profile, and
...
choosing. The last two layers are both the operator's, and they are ordered by
how specific the instruction is: a stored definition beats an organization entry
of the same name, because the deployment operator owns the gateway and this
process has already built the guardrail, and a routing policy beats both,
because it names an endpoint on purpose. A profile an outer layer claims loses
the credential an inner one carried, because that credential was stored for the
endpoint the inner entry named.
