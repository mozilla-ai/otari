...
provider: whether a file is staged follows who runs the code. A workspace pin is
applied later than staging, so a pin that pulls a natively served declaration
here runs that request without its attachments seeded.
