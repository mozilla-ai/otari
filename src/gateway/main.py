...
# schemes this stamps everything else with). Named as full leaf paths rather than
# a shared `/v1/auth` or `/v1/invitations` prefix on purpose: a prefix would
# exempt any future route mounted under it too, by inheritance rather than by
# decision (an operator-only resend or list-pending endpoint, say — `/v1/auth/
# session` and `/v1/auth/password` both live under `/v1/auth` and do require a
# credential). Listed separately from _PUBLIC_PREFIXES because these still get
