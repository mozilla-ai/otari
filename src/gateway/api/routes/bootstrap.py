...
class DeploymentBootstrap(BaseModel):
    ...
    data_plane_url: str | None = Field(
        description=(
            ...
            "target like management_url: it is the gateway's own address, which the dashboard "
            "suffixes with /v1 to build its request snippets, so a value already ending in /v1 "
            "renders a snippet posting to /v1/v1 (this gateway refuses one at startup, and any "
            "deployment serving this contract should publish the bare address). Null on a hosted "
