...
def merge_guardrail_layers(
    ...
    what the deployment's routing policy mandates, then the deployment's own
    stored definitions scoped to it.

    The last two are both the operator's, and the stored definition is the
    outermost because it is the more explicit instruction: the operator built
    this guardrail in this process and switched it on for this workspace. So
    where a policy or an organization entry names the same profile and an
    endpoint, the check runs here rather than being sent there.
    ...
    for layer in (policy, deployment):
        if not layer:
            continue
