...
class CheckPassedGate:
    ...
    gate can genuinely block. A repo-local script is the same trust level as
    a Makefile target or a pre-commit hook already checked into the repo:
    running ``otari hook`` against a branch you do not already trust carries
    the same risk running any of its other scripts would, not a new risk
    this gate type introduces. There is deliberately no guard requiring the
    verifier to predate the diff under check, and no sandboxing: a "must
    predate this diff" rule was considered and rejected because it breaks
    the primary workflow this gate type is for, someone writing a new
    verifier and using it in the same change. See docs/agent-gates.md.
