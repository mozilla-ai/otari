...
async def sweep_files(db: AsyncSession, file_store: FileStore, *, batch_size: int) -> int:
    ...
    short. Does not commit: the caller's Unit of Work block does. A blob that
    is already gone is not an error (the delete route removes the bytes
    best-effort before this ever sees the row); any other storage failure
    leaves the row in place for the next pass rather than orphaning bytes
    nothing references.
    """
        ...
        try:
            async with create_session() as db:
                ...
                for _ in range(_MAX_SWEEP_PASSES):
                    async with uow:
                        swept = await sweep_files(db, file_store, batch_size=batch_size)
                    if swept < batch_size:
                        break
