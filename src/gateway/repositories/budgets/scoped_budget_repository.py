...
class ScopedBudgetRepository(BaseRepository[ScopedBudget, Never, Never]):
    ...
    async def insert_member_ceilings(self, ceilings: Sequence[ScopedBudget]) -> list[ScopedBudget]:
        ...
        for ceiling in ceilings:
            scope_id, provider_key_id = ceiling.scope_id, ceiling.provider_key_id
            try:
                async with self.db.begin_nested():
                    self.db.add(ceiling)
                    await self.db.flush()
            except IntegrityError:
                # Asked of the database, because the engines word the refusal differently.
                if not await self.has_ceiling(SCOPE_WORKSPACE_MEMBER, scope_id, provider_key_id):
                    raise
                continue
