...
def _hook_run_check_verifier(repo_root: Path, verifier: str, *, deadline: float) -> tuple[str, str]:
    ...
    author is expected to write the human-readable reason to.
    """
    remaining = deadline - time.monotonic()
    if remaining <= 0:
    ...
    try:
        result = subprocess.run(  # noqa: S603 - no shell, resolved path checked against repo_root above
            [str(script_path)],
            cwd=repo_root,
            capture_output=True,
            text=True,
        ...
        return "error", f"verifier did not respond within {min(_HOOK_CHECK_TIMEOUT_SECONDS, remaining):.0f}s"
    ...
    detail = result.stdout[:_HOOK_MAX_CHECK_DETAIL_LENGTH]
    if result.returncode == 0:
        return "pass", detail
    if result.returncode == 1:
        return "fail", detail
    return "error", detail or f"verifier exited with status {result.returncode}"
