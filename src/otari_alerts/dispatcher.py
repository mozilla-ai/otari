...
This module knows nothing about budgets. It takes a destination URL, a title
and a body, and reports whether the send worked.

**Why Apprise.** One string covers Slack, PagerDuty, mail and a plain webhook,
so the schema is one column rather than one per vendor. There is no port over
it because Apprise is itself the vendor-neutral layer, so a port would be a
seam with one possible implementation on either side.

**Threads, not the event loop.** ``Apprise.async_notify`` is a coroutine but
its plugins are synchronous ``requests`` calls it offloads with
...
class UnsupportedAlertDestinationError(ValueError):
    ...
    Raised at rule-write time, so an operator learns their URL is unusable
    while the form is open rather than when a budget crosses a threshold weeks
    later.
    """
...
class ParsedDestination:
    ...
    the plugin has none because its endpoint is compiled in. It is the value
    the SSRF gate checks; deciding *whether* to check it is the caller's job,
    since the netloc of a vendor schema is a token rather than a host.
    """
...
def parse_destination(destination: str) -> ParsedDestination:
    ...
    ``suppress_exceptions`` keeps a malformed URL from surfacing as whatever
    the plugin's constructor happened to raise, so every rejection reaches the
    caller as one domain error.

    ``smtp_host`` before ``host`` because Apprise's mail plugin maps a
    well-known domain onto its provider's server (``mailto://u:p@gmail.com``
    sends to ``smtp.gmail.com``), and the gate has to check the address that is
    dialed.

    Raises:
        UnsupportedAlertDestinationError: If the string is blank, or Apprise
            cannot instantiate a plugin for it.
...
def _bound_sockets(client: apprise.Apprise) -> None:
    ...
    The bound is per socket operation, not per send, because Apprise may retry
    a plugin. Closing that gap would mean owning Apprise's retry loop.
    """
