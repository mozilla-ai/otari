...
and dispatches. :func:`run_budget_alert_evaluator` is what
:func:`otari_alerts.register` contributes as a background task, so Otari's
lifespan starts it beside its own refreshers.

**Why a periodic evaluator rather than a hook on the request path.** The
budget-refusal sites in ``gateway.services.budget_service`` sit inside the
...
absorb either, to buy latency a budget warning does not need. Reading state on
a timer costs the request path nothing, makes "warning" and "exceeded" the same
...
matching the gate in ``gateway.services.scoped_budget_service.reserve``.
Measuring committed spend alone would report 90 percent on a ceiling that is
...
from otari_alerts.config import settings
from otari_alerts.dispatcher import send_alert
from otari_alerts.metrics import record_alert_delivery
from otari_alerts.models import AlertDelivery, AlertRule

# Kinds of alert, and the dedupe key's ``kind`` column. Strings rather than an
# enum column, per :class:`otari_alerts.models.AlertDelivery`.
...
def _percent(used: Decimal, cap: Decimal) -> int:
    ...
    Floored so 79.9 percent does not fire an 80 percent warning; clamped
    because a cap lowered under an already-spent ceiling gives an arbitrary
    ratio.
    """
    if cap <= 0:
        return 0
...
def _format_amount(axis: str, value: Decimal) -> str:
    ...
    A Decimal would otherwise render a token count of ``1000`` as
    ``1000.000000``.
    """
    if axis == "spend":
        return f"${value:.2f}"
...
async def _load_ceilings(db: AsyncSession, organization_ids: set[uuid.UUID]) -> list[tuple[ScopedBudget, Budget]]:
    ...
    carries no tenant of its own, so its owner is the ``organization_id`` on
    the ``budgets`` row it names. A NULL one is the deployment's own and is
    excluded by the ``IN``.
...
async def purge_delivered_before(db: AsyncSession, *, older_than: datetime) -> int:
    ...
    retention rather than cascade is what bounds the table. Safe to run during
    a pass: a row old enough to purge belongs to a period that has long rolled.
    """
...
async def run_budget_alert_evaluator(config: GatewayConfig) -> None:
    """Evaluate budget alerts forever. Cancelled at shutdown.

    The contributed background task. Otari's lifespan starts it in every mode
    and hands it the ``GatewayConfig``, but the interval is the plugin's own
    setting rather than a field on that config, so ``config`` is read only for
    the one thing Otari knows and the plugin does not: whether this process has
    a local database at all. A hybrid gateway reserves nothing locally and has
    no ``scoped_budgets`` to read, so the worker returns instead of failing a
    query every tick.
    ...
    """
    interval = settings.alert_evaluation_interval_sec
    if config.is_hybrid_mode:
        logger.info("Budget alerts: no local database in hybrid mode, evaluator not started")
        return
    if interval <= 0:
        logger.info("Budget alerts: OTARI_ALERT_EVALUATION_INTERVAL_SEC is 0, evaluator not started")
        return
