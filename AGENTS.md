...
- **Adding a table or a cross-repo surface** → **there is no per-PR reconciliation record to file.** Not an issue, not a comment, not a note in a doc. Where a surface lands across this gateway and otari.ai is decided in [otari-ai's `docs/architecture/m4-reconciliation.md`](https://github.com/mozilla-ai/otari-ai/blob/main/docs/architecture/m4-reconciliation.md); open a PR against that page only when you change a decision it states. If your change leaves a real question unanswered, put it in the PR description and let a human decide whether it becomes an issue.
