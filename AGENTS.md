...
- Otari's own HTTP headers are named `Otari-*`, never `X-Otari-*`. RFC 6648 (BCP 178) says creators of new parameters "SHOULD NOT prefix their parameter names with 'X-' or similar constructs": https://datatracker.ietf.org/doc/html/rfc6648. No lint rule or type error reaches it, so `tests/unit/test_header_naming.py` fails a change that reintroduces the prefix anywhere under `src/gateway`. It scans case-sensitively, which is what keeps an OpenAPI specification extension out of scope: `x-otari-optional` in `docs/public/code-execution-openapi.yaml` is lowercase and that spec requires the `x-`. The rule covers headers Otari defines; whether the ones it inherited from a peer's contract (`X-Gateway-Token`, `X-User-Token`, `X-Correlation-ID`) should follow it is open in #1486.
