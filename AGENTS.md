...
- **Adding a table or a cross-repo surface** → the decisions about which side of the otari/otari.ai reconciliation a surface lands on live in [otari-ai's `docs/architecture/m4-reconciliation.md`](https://github.com/mozilla-ai/otari-ai/blob/main/docs/architecture/m4-reconciliation.md). If your change contradicts a decision there or makes a new one, open a PR against that page. If it raises a question nobody has answered, open an issue in the `otari-migration-m5` milestone. Neither is a per-PR record: a change an existing decision already covers goes in the PR description and nowhere else.
