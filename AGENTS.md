...
- A docblock belongs immediately above the declaration it describes, and the way it stops doing so is an insertion. When you add a declaration beside an existing documented one, put the new pair above or below the whole block, never between a block and what it documents, and do not move the existing declaration to make room: moving it is what opens the gap the next insertion falls into. Four of these are in the tree, three of them written in one day, and none is reachable by lint, typecheck or any test, because a comment over the wrong thing compiles. `grep -Pzo '\*/\n\s*/\*\*'` finds the shape, two docblocks with nothing between them, but a file header followed by the first declaration's own block matches it too, so the hits want reading rather than fixing and this is a triage aid rather than a gate.
