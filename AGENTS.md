...
- **Hosted** (`OTARI_MODE=hosted`) is standalone's multi-tenant variant, not a third runtime: same database, same routers, same sign-in, and `is_hybrid_mode` is false. It differs only in what `GET /v1/bootstrap` publishes, `deployment_type: "hosted"` and `HOSTED_SURFACES` rather than `STANDALONE_SURFACES`, which drops the process-global `providers` page (`provider_credentials` is keyed on instance name alone, so one row serves every tenant) and adds the organization-scoped `organization_providers` one. Hiding a page is not a guard over the table; #818 tracks that.
