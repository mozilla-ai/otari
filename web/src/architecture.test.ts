...
function lintProbe(layer: string, source: string): Diagnostic[] {
  ...
  try {
    ...
    stdout = execFileSync(process.execPath, [BIOME, "lint", "--reporter=json", file], {
      cwd: WEB,
      encoding: "utf8",
...
describe("layer boundaries", () => {
  it("reject a feature importing the composition root", () => {
    const messages = rejects("features", 'import { router } from "@/app/router";\nexport const a = router;\n');
  ...
  it.each(["app", "features", "shared"])("reject %s importing the overlay tree", (layer) => {
    ...
    const messages = rejects(layer, 'import { nav } from "@/overlay/nav";\nexport const a = nav;\n');
    expect(messages).toHaveLength(1);
...
describe("the boundary check is wired up", () => {
  ...
  it("runs in the dashboard workflow", () => {
    const workflow = join(WEB, "..", ".github", "workflows", "otari-dashboard.yml");
