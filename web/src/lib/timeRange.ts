...
export function findPreset(presets: RangePreset[], key: string): RangePreset | undefined {
...
export function bucketForWindow(startIso: string, endIso: string | undefined, now: number = Date.now()): UsageBucket {
...
export function formatWindowLabel(startIso: string | undefined, endIso: string | undefined): string {
  if (!startIso && !endIso) {
    return "All time";
  }
  if (startIso && !endIso) {
  ...
  const sameYear = new Date(startMs).getUTCFullYear() === new Date(endMs).getUTCFullYear();
