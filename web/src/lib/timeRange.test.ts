...
describe("bucketForWindow / bucketDurationMs", () => {
  ...
  it("buckets a sub-day (or exactly one-day) window hourly, longer daily", () => {
    expect(bucketForWindow("2026-07-26T00:00:00Z", "2026-07-27T00:00:00Z", now)).toBe("hour");
    expect(bucketForWindow("2026-07-20T00:00:00Z", "2026-07-27T00:00:00Z", now)).toBe("day");
  });
...
describe("rangeFromBuckets", () => {
  const starts = ["2026-07-01T00:00:00Z", "2026-07-02T00:00:00Z", "2026-07-03T00:00:00Z", "2026-07-04T00:00:00Z"];

  it("spans from the first picked bucket to the exclusive end of the last", () => {
    const r = rangeFromBuckets(starts, 1, 2, "day");
    expect(r).toEqual({ startIso: "2026-07-02T00:00:00.000Z", endIso: "2026-07-04T00:00:00.000Z" });
  });

  it("is order-independent and clamps to the series bounds", () => {
    const r = rangeFromBuckets(starts, 9, -3, "day");
    expect(r).toEqual({ startIso: "2026-07-01T00:00:00.000Z", endIso: "2026-07-05T00:00:00.000Z" });
  });
...
describe("bucketIndexRange", () => {
  ...
  it("spans the whole series when the window covers it (or is absent)", () => {
    expect(bucketIndexRange(starts, undefined, undefined)).toEqual({ startIndex: 0, endIndex: 4 });
    expect(bucketIndexRange(starts, "2026-06-01T00:00:00Z", "2026-08-01T00:00:00Z")).toEqual({
      startIndex: 0,
      endIndex: 4,
    });
  });
  ...
  it("locates a sub-window by its bucket boundaries", () => {
    ...
    expect(bucketIndexRange(starts, "2026-07-02T00:00:00Z", "2026-07-04T00:00:00Z")).toEqual({
      startIndex: 1,
      endIndex: 2,
    });
  });
  ...
  it("clamps an inverted or empty window and handles an empty series", () => {
    const r = bucketIndexRange(starts, "2026-07-04T00:00:00Z", "2026-07-02T00:00:00Z");
    expect(r.endIndex).toBeGreaterThanOrEqual(r.startIndex);
    expect(bucketIndexRange([], "2026-07-01T00:00:00Z", "2026-07-02T00:00:00Z")).toEqual({ startIndex: 0, endIndex: 0 });
  });
});
...
describe("formatWindowLabel", () => {
  ...
  it("labels an open-ended (start-only) window", () => {
    expect(formatWindowLabel("2026-07-03T00:00:00Z", undefined)).toContain("Since");
    expect(formatWindowLabel("2026-07-03T00:00:00Z", undefined)).toContain("Jul 3");
  });

  it("shows the inclusive last day for a bounded range, not the exclusive bound", () => {
    const label = formatWindowLabel("2026-07-03T00:00:00Z", "2026-07-11T00:00:00Z");
  ...
  it("collapses a single-day window to one date", () => {
    const label = formatWindowLabel("2026-07-03T00:00:00Z", "2026-07-04T00:00:00Z");
    expect(label).toContain("Jul 3");
    expect(label).not.toContain("–");
  });

  it("carries the year on both sides across a year boundary", () => {
    const label = formatWindowLabel("2025-12-30T00:00:00Z", "2026-01-03T00:00:00Z");
