...
describe("UsersPage", () => {
  ...
  it("hides auto-created virtual users until the toggle is enabled", async () => {
    ...
    await user_.click(
      screen.getByRole("checkbox", { name: /Show auto-created/ }),
    )
  ...
  it("creates a user with an assigned budget", async () => {
    ...
    const post = fetchMock.mock.calls.find(
      ([u, init]) =>
        String(u).endsWith("/v1/users") && (init?.method ?? "") === "POST",
    )
  ...
  it("blocks a user via PATCH, then offers to unblock", async () => {
    const fetchMock = mockApi({
      users: [user({ user_id: "alice", blocked: false })],
    })
    const user_ = userEvent.setup()
    renderPage(<UsersPage />)
  ...
  it("bulk-deletes the selected users after a confirm", async () => {
    const fetchMock = mockApi({
      users: [user({ user_id: "alice" }), user({ user_id: "bob" })],
    })
    const user_ = userEvent.setup()
    renderPage(<UsersPage />)
  ...
  it("bulk-assigns a budget to the selected users", async () => {
    ...
    const bar = (await screen.findByText("1 selected")).closest("div")!
    await user_.click(
      within(bar).getByRole("button", { name: "Assign budget" }),
    )
