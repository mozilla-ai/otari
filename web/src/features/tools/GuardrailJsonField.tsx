  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <FieldMessages reserve>
        <Description
          className={
            error || parsed.error ? "text-caption text-danger" : "text-caption"
          }
        >
          {/* The registry's line while the controls are up, and the catalog's
              own in the JSON view: a paragraph on which dict to pass is what an
              operator needs there and exactly what the checkboxes replaced. */}
          {error ??
            parsed.error ??
            (view === "fields" && suggestion?.help ? (
              suggestion.help
            ) : view === "json" && spec.description ? (
              <DocstringText full>{spec.description}</DocstringText>
            ) : (
              description
            ))}
        </Description>
