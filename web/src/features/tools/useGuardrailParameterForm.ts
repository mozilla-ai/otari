 ...
 * keeps the seeding rule in one place: an existing entry and a new one seed
 * from different sources but must both re-seed when the schema arrives, and a
 * catalog that loads a moment after the card does is the ordinary case rather
 * than the edge one.
 */
