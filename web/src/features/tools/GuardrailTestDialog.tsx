...
import { useTestGuardrailDefinition } from "@/shared/api/tools"

// Run one stored guardrail against a sample input, so a definition can be
// checked before any traffic depends on it.
//
// A disabled guardrail is testable too, which is most of the point of having
// this: checking a definition is how an operator decides whether to turn it on.

// The route's own ceiling, not a guess, so the field refuses what the API would
...
export function GuardrailTestDialog({
  ...
  const test = useTestGuardrailDefinition()
  ...
  return (
    <FormDialog
      ...
      // No dirty guard: a sample input is not work to lose, and the prompt would
      // sit between the operator and the result they just asked for.
      error={test.error}
    >
      <TextArea
        label="Sample input"
