...
const FIELDS: Record<string, JsonFieldSpec> = {
  ...
  "alinia.detection_config": {
    kind: "flags",
    help: "Which detections Alinia runs.",
    itemNoun: "detection",
    options: [
      {
        key: "security",
  ...
  "watsonx_guardian.detectors": {
    kind: "flags",
    help: "Which detectors watsonx runs.",
    itemNoun: "detector",
    options: [
      {
  ...
  "patronus.evaluators": {
    kind: "presets",
    help: "Which checks Patronus runs. At least one.",
    itemNoun: "evaluator",
    options: [
      {
...
/**
 * Whether a documented key asks this guardrail for that job.
 *
 * The positive half of the pair below, and it can reach past the catalog's own
 * `categories`: watsonx Guardian documents a `pii` detector while its metadata
 * lists no `pii` category, so the guardrail can do the job and upstream does
 * not say so. A key that is documented is better evidence than a category that
 * is missing.
 */
export function mappedFor(
  guardrailName: string,
  ...
  return parameters.some(
    (parameter) =>
      jsonFieldSpec(guardrailName, parameter)?.byCategory?.[category] !==
      undefined,
  )
}

/**
 * Whether this guardrail can be set up for that job from here.
 *
 * False only where the guardrail's own configuration turns on a vocabulary this
 * build has, and the job is not in it. Alinia and Patronus both declare that
 * they detect personal data, and neither any-guardrail's source, its reference
 * nor the vendor walkthrough names the detection or the evaluator that does it,
 * so the form can offer no way to ask for it. A picker that offers them anyway
 * is offering a dead end.
 *
 * A guardrail with no such field is always configurable: Lakera Guard and
 * Bedrock Guardrails are told what to check by their API key and their
 * AWS-side configuration, so there is nothing here for an operation to be
 * missing from.
 */
export function configurableFor(
  guardrailName: string,
  parameters: readonly GuardrailParameterSpec[],
  category: string,
): boolean {
  ...
  for (const parameter of parameters) {
    const field = jsonFieldSpec(guardrailName, parameter)
    if (field?.byCategory === undefined) continue
    if (field.byCategory[category] === undefined) return false
  }
  return true
}
