...
function row(
  overrides: Partial<UsageGroupRow> & { key: string | null },
): UsageGroupRow {
  return {
    label: null,
    cost: 0,
...
describe("cardModels", () => {
  it("ranks by tokens, not cost, so a cheap high-volume model is not buried", () => {
    ...
    const rows = [
      row({ key: "gpt-4o", cost: 42, tokens: 1_000 }),
      row({ key: "llama-3.3-70b", cost: 0, tokens: 9_000 }),
    ]
    expect(cardModels(rows).map((m) => m.key)).toEqual([
      "llama-3.3-70b",
      "gpt-4o",
    ])
  })
  ...
  it("collapses each row's model name to the final model type", () => {
    const rows = [
      row({ key: "otari.ai:fireworks/accounts/deepseek-v4-flash", tokens: 10 }),
    ]
...
describe("collapseModelName", () => {
  it("keeps only the final model type from a fully-qualified selector", () => {
    expect(
      collapseModelName("otari.ai:fireworks/accounts/deepseek-v4-flash"),
...
describe("availableStats", () => {
  ...
  it("omits a $0.00 cost, which is the normal state for a self-hosted gateway", () => {
    const stats = availableStats({
      totals: totals({ cost: 0, request_count: 5 }),
      series,
      hideDollars: false,
    })
  ...
  it("does not caveat cost when the gateway reports zero unpriced requests", () => {
    const stats = availableStats({
      totals: totals({ cost: 12, unpriced_requests: 0 }),
      series,
      hideDollars: false,
    })
  ...
  it("drops cost entirely when dollars are hidden", () => {
    const stats = availableStats({
      totals: totals({ cost: 12, request_count: 3 }),
      series,
      hideDollars: true,
    })
    expect(stats.map((s) => s.id)).not.toContain("cost")
  ...
  it("formats latency in seconds past a second", () => {
    const stats = availableStats({
      totals: totals({ avg_latency_ms: 2400 }),
      series,
      hideDollars: false,
    })
...
describe("resolveHero", () => {
  it("promotes the next available stat rather than leaving a hole", () => {
    ...
    const stats = availableStats({
      totals: totals({ cost: 12, request_count: 7 }),
      series,
      hideDollars: true,
    })
