...
vi.mock("@/features/usage/shareImage", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/features/usage/shareImage")>()
  return {
    ...actual,
...
describe("ShareDialog", () => {
  ...
  it("offers download as the terminal action, with no post-composition surface", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Post on X" }),
    ).not.toBeInTheDocument()
    expect(
      screen.queryByLabelText("Suggested post text"),
    ).not.toBeInTheDocument()
  })
  ...
  it("does not explain the clipboard's origin rules", () => {
    renderDialog()
    expect(
      screen.queryByText(/secure \(https\) origin/i),
    ).not.toBeInTheDocument()
  })
  ...
  it("does not offer cache hit rate as the card's lead", () => {
    ...
    renderDialog()
    expect(
      screen.queryByRole("button", { name: "Cache hits" }),
    ).not.toBeInTheDocument()
  })
  ...
  it("carries no window or filter control: data scope comes from the page", () => {
    ...
    expect(
      screen.getByText(/window and filters currently applied above/i),
    ).toBeInTheDocument()
  })
  ...
  it("blocks the actions while the page's numbers are still in flight", () => {
    ...
    expect(
      screen.getByText(/Waiting for the current numbers/i),
    ).toBeInTheDocument()
  })

  it("does not claim a copy that failed", async () => {
    const user = userEvent.setup()
    const shareImage = await import("@/features/usage/shareImage")
  ...
  it("falls back per field when stored presentation holds an unknown value", () => {
    ...
    localStorage.setItem(
      "otari.share.presentation.v1",
      JSON.stringify({ ratio: "portrait", rows: 4, theme: "neon" }),
    )
    renderDialog()
    expect(
      screen.getByRole("button", { name: "Download PNG" }),
