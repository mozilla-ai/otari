    ...
    () => [
      ...
      },
      {
        ...
        cell: (policy) => {
          ...
          // An em dash where a policy has no readiness to report, rather than an
          // empty cell: a fallback chain has nothing to learn, which is an
          // absence worth stating and not the same as zero examples.
          const readiness =
      ...
      <header className="flex flex-col gap-4 pb-1 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-[620px]">
          ...
          <p className="mt-1 text-sm text-muted">
            ...
            which guardrails always run. It can also split traffic across
            providers by weight, or let a router learn which prompts a cheaper
            model handles just as well.
          </p>
        </div>
        {adding || editing !== null ? null : (
          <Button
            variant="primary"
            className="shrink-0"
            onPress={() => {
