...
describe("copyToClipboard", () => {
  ...
  it("copies on a non-secure origin, where there is no Clipboard API at all", async () => {
    ...
    expect(await copyToClipboard("anthropic:claude-opus-4-5-20251101", undefined)).toBe(true);
    expect(execCommand).toHaveBeenCalledWith("copy");
  });
  ...
  it("does not report success when both paths fail", async () => {
    ...
    expect(await copyToClipboard("provider exploded", { writeText })).toBe(false);
  });
