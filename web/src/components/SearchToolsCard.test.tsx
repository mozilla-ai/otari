...
const PROVIDERS: SearchProviderInfo[] = [
  { id: "exa", requires_api_key: true, requires_api_base: false, default_api_base: "https://api.exa.ai" },
  { id: "searxng", requires_api_key: false, requires_api_base: true, default_api_base: "http://searxng:8080" },
];
...
const TOOLS: SearchToolsResponse = {
  ...
  config: [{ name: "from-file", provider: "exa", api_base: null, has_api_key: true, shadowed: false }],
};
    ...
    if (url.includes("/v1/search-tools")) {
      if (method !== "GET") {
        if (opts.writeStatus && opts.writeStatus >= 400) {
          return jsonResponse({ detail: opts.writeDetail ?? "bad" }, opts.writeStatus);
        }
        return jsonResponse({ name: "new", provider: "searxng" }, method === "POST" ? 201 : 200);
      }
...
describe("SearchToolsCard", () => {
  ...
  it("lists stored tools as editable and config-file tools as read-only", async () => {
    ...
    expect(screen.getByLabelText("Backend URL for local")).toHaveValue("http://searxng:8080");
    expect(screen.getByText("from-file")).toBeInTheDocument();
    expect(screen.getByText("Config file")).toBeInTheDocument();
    // A config-file tool has no editable box of its own.
    expect(screen.queryByLabelText("Backend URL for from-file")).not.toBeInTheDocument();
  });
  ...
  it("adds a tool with the chosen provider", async () => {
    ...
    await user.type(screen.getByLabelText("Search backend URL"), "http://other:8080");
  ...
  it("surfaces a rejected write next to the tool", async () => {
    mockApi({ writeStatus: 422, writeDetail: "search_tools.local.api_key is required for provider 'exa'." });
    const user = userEvent.setup();
    renderWithClient(<SearchToolsCard onSaved={() => {}} />);
    await screen.findByText("local");
