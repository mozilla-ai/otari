...
test.describe("overview", () => {
  test("reports the gateway's own totals once it has traffic", async ({ page }) => {
    await login(page);
  ...
  test("previews the newest requests and opens the full log", async ({ page }) => {
    await login(page);
  ...
  test("the budget tile is a way into budgets, not just a number", async ({ page }) => {
    await login(page);
