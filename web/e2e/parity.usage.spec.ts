...
test.describe("usage & analytics", () => {
  test("reports the window's spend, tokens, cache and latency", async ({ page }) => {
  ...
  test("the chart changes what it measures and how it is split", async ({ page }) => {
    await openUsage(page);
    ...
    await expect(page.getByText(PARITY.models.priced.model, { exact: true }).first()).toBeVisible();
  ...
  test("breaks spend down by model and by user", async ({ page }) => {
    ...
    await expect(breakdownRow(page, "model", PARITY.models.priced.model)).toBeVisible();
    await expect(breakdownRow(page, "model", PARITY.models.priced.model)).toContainText(
      String(COUNTS.priced),
    );

    await page.getByRole("button", { name: "User", exact: true }).first().click();
    await expect(page.getByRole("heading", { name: "Spend by user" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Spend by session" })).toBeVisible();
    await expect(breakdownRow(page, "session", PARITY.sessions.heavy)).toBeVisible();

    await page.getByRole("button", { name: "Provider", exact: true }).click();
    await expect(page.getByRole("heading", { name: "Spend by provider" })).toBeVisible();
    await expect(breakdownRow(page, "provider", PARITY.models.priced.provider)).toBeVisible();
  });

  test("drills from a model's spend into the requests behind it", async ({ page }) => {
    await openUsage(page);
    await breakdownRow(page, "model", PARITY.models.priced.model).getByRole("rowheader").click();
  ...
  test("drills from a session into the requests it ran", async ({ page }) => {
    await openUsage(page);
    await breakdownRow(page, "session", PARITY.sessions.light).getByRole("rowheader").click();

    await expect(page.getByRole("heading", { name: "Activity" })).toBeVisible();
    await expect(filterChip(page, "Session", PARITY.sessions.light)).toBeVisible();
    // The light session carries both its succeeding and its failing requests.
    await expect(tableRows(page, "Activity log")).toHaveCount(COUNTS.unpriced + COUNTS.errors);
    await expect(table(page, "Activity log")).toContainText(PARITY.models.unpriced.model);
  });
});
