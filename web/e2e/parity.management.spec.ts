...
async function openPage(page: Page, link: string, heading: string): Promise<void> {
...
function row(page: Page, ariaLabel: string, name: string | RegExp): Locator {
  return tableRows(page, ariaLabel).filter({ has: page.getByRole("rowheader", { name }) });
}
...
async function fillModelBox(page: Page, name: RegExp, value: string): Promise<void> {
...
test.describe("standalone provider setup", () => {
  test("adds a provider, reports what testing it found, edits and removes it", async ({ page }) => {
    await login(page);
    ...
    const deleteProvider = row(page, "Providers", PROVIDER).getByRole("button", { name: "Delete" });
...
test.describe("api keys", () => {
  test("issues a key, and will not delete one until it is disabled", async ({ page }) => {
    await login(page);
...
test.describe("budgets", () => {
  test("creates a budget against a user, edits its limit, and removes it", async ({ page }) => {
    await login(page);
    ...
    await expect(page.getByRole("button", { name: `Remove ${PARITY.users.heavy}` })).toBeVisible();
    await page.getByRole("button", { name: "Create budget" }).click();
    ...
    await row(page, "Budgets", BUDGET).getByRole("button", { name: "History" }).click();
